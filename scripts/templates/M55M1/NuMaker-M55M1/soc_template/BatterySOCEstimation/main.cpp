 ...
 * @file     main.cpp
 * @version  V1.00
 * @brief    Battery SOC estimation sample.
 *
...
#include "Pattern/SOC_test_data.h"
...
#define EACH_PERF_SEC 5
    ...
    float *pfInputTestData = (float *)test_x_seq;
    float *pfOutputTestData = (float *)test_y_seq;

    //time steps * (Voltage, Current, Temperature, SOH)
    int32_t i32InputDataSize = test_x_seq_dim[1] * test_x_seq_dim[2];
    int32_t i32InputDataIndex = 0;
    int32_t i32InputDataItems = test_x_seq_dim[2];
    int32_t i32OutputDataIndex = 0;
		int32_t i32Cycle = 0;
		int32_t i32ToatlCycles = test_x_seq_dim[0];
