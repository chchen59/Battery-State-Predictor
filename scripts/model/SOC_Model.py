...
def SOCModel(workspace, dataset_path, train_data, test_data, epochs):
    ...
    mdh = ModelDataHandler(dataset, [
        DischargeCycleCols.VOLTAGE_MEASURED,
        DischargeCycleCols.CURRENT_MEASURED,
        DischargeCycleCols.TEMPERATURE_MEASURED,
        DischargeCycleCols.SOH,
    ])

    train_x, train_raw_x, train_y, test_x, test_raw_x, test_y = mdh.get_discharge_whole_cycle(multiple_output=True, soh=False, output_capacity=False)
